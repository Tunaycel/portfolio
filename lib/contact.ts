...
export function validateContact(value: unknown): Contact | null {
  ...
  if (typeof v.name !== "string" || typeof v.email !== "string" || typeof v.message !== "string")
    return null;
...
export async function handleContact(
  request: Request,
  { config, fetcher, allow }: Dependencies,
): Promise<Response> {
  const origin = request.headers.get("origin");
  let expected: string;
  try {
    const target = new URL(config.origin || request.url);
    // Next can normalize the internal request URL to localhost. In local/unconfigured
    // deployments, match the actual Host header; production uses the explicit origin.
    if (!config.origin && request.headers.get("host")) {
      target.host = request.headers.get("host")!;
    }
    expected = target.origin;
  } catch {
    return reply(503, "Contact is unavailable. Please email h.tunaycelik@gmail.com.");
  }
  ...
  if (
    ...
    return reply(415, "Please send a JSON request.");
  let data: Contact | null;
  try {
    data = validateContact(await readBoundedBody(request));
  } catch (error) {
    return reply(
      error instanceof RangeError ? 413 : 400,
      "Please check the message size and format.",
    );
  }
  if (!data) return reply(400, "Enter a name, a valid email and a message of 20–4,000 characters.");
  if (data.website)
    return reply(400, "The message could not be accepted. Please email me directly.");
  if (!config.apiKey || !config.from || !config.to)
    return reply(503, "The contact form is unavailable. Please email h.tunaycelik@gmail.com.");
  ...
  if (!allow(key))
    return reply(429, "Too many messages. Please wait 10 minutes or email me directly.");
  try {
    ...
    if (
      ...
      return reply(502, "Delivery could not be confirmed. Please email me directly.");
    return reply(
      200,
      "Your message has been accepted for delivery. Thank you for getting in touch.",
    );
  } catch {
    return reply(502, "Delivery could not be confirmed. Please email me directly.");
  }
